
import java.util.Random;

import javax.swing.JOptionPane;

public class Validation {
	public static void main(String[] args) {

		// 1. Watch the first 2 minutes of this movie: http://vimeo.com/2485018
		
		// 2. On paper, write all the numbers that get printed when you run this class
		//System.out.println(randomNumber);

		// 3. Use the randomNumber to give the user a random compliment.
		for (int i = 0; i < 10; i++) {
			int randomNumber = new Random().nextInt(5);
		
		if(randomNumber==(0)){
			JOptionPane.showMessageDialog(null, "You have a nice face!!!!!");
		}
		if(randomNumber==(1)){
			JOptionPane.showMessageDialog(null, "You are something special!");
		}
		else if(randomNumber==(2)){
			JOptionPane.showMessageDialog(null, "You are almost as cool as June!!!");
		}
		else if (randomNumber==(3)){
			JOptionPane.showMessageDialog(null, "Your pretty....pretty oh wait let me not finish that!");
		}
		else if (randomNumber==(4)){
			JOptionPane.showMessageDialog(null, "You have good style!!!");
		}
		else if (randomNumber==(5)){
			JOptionPane.showMessageDialog(null, "You have a funny look on your face!!!!");
		}}
		// 4. Repeat all the code above 10 times
		
		// 5. Find someone to test out your program. They will like it :)
	}
}








































